...
reverse_good_log = function(x) {
  ...
  y = ifelse(abs(y) > 40,  sign(y) * 40, y)

    return(y)
}

cross_val_d3 <- function(model_def, tx) {
...
model3_grammar <- CreateGrammar(list(
...
cross_val_d3.m = memo_fitness_function(cross_val_d3)

ev3 = GrammaticalEvolution(model3_grammar, function(expr) cross_val_d3.m(eval(expr), sp3), monitorFunc = print, iterations = 80)
save(ev3, file="models/d3.RData")
